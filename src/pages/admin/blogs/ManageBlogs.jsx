...
const [blogData, setBlogData] = useState([]);
  ...
  const handlePage = (value=1) => {
      ...
      const totat_page = Math.ceil(blogData.length/item_per_page);
      if (value < 1  || value>totat_page) return ;
     const filtered_data= blogData.slice((value-1)*item_per_page,((value-1)*item_per_page)+5 );
     setFilteredData(filtered_data);
     setPage(value)
  ...
  return (
    <div className='h-full w-[calc(100vw-177px)] relative p-10'>
        <div className="flex justify-end pb-5">
            <button onClick={()=>navigate('add')} className='text-white bg-primary py-4 px-6 rounded-md focus:scale-90 duration-100 font-[500] '>
                Add New Blogs
            </button>
            </div>
            <div className=''>
            ...
            <tbody className='bg-grayDark'>
              {blogData.length > 0 ? (
                ...
                <div className={`h-2 ${(blogData?.length-1)%2===0?'bg-[#101726]':'bg-[#17212E]'} rounded-b-md`}></div>
            </div>
